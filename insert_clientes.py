...
def clean_contrato(contrato):
    ...
    contrato['codigo'] = clean_int(contrato['codigo'])
    contrato['valor'] = clean_float(contrato['valor'])
    ...
    contrato['id_cliente'] = clean_int(contrato['id_cliente'])
    contrato['id_dentista'] = clean_int(contrato['id_dentista'])
...
def clean_mensalidade(mensalidade):
    ...
    mensalidade['status'] = clean_str(mensalidade['status'])
    mensalidade['vencimento'] = clean_date(mensalidade['vencimento'])
    mensalidade['valor'] = clean_float(mensalidade['valor'])
    mensalidade['multa'] = clean_float(mensalidade['multa'])
    mensalidade['pagamento'] = clean_float(mensalidade['pagamento'])
    mensalidade['desconto'] = clean_float(mensalidade['desconto'])
    mensalidade['residuo'] = clean_float(mensalidade['residuo'])
...
def clean_tratamento(tratamento):
    ...
    tratamento['id_dentista'] = clean_int(tratamento['id_dentista'])
...
if __name__ == '__main__':
    for obj in os.listdir('data/clientes/'):
        cliente = load_obj('clientes', obj)
        command = insert_cliente(cliente)
